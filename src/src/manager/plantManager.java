...
public class PlantManager {
    ...
    public void alertPlant(TileManager tileManager, Zombie_fakeManager zombieFakeManager){
        for(int i = 0;i<tileManager.getTiles().length;i++){
            Rectangle r = tileManager.getTiles()[i].getBound();
            if(r.contains(zombieFakeManager.getZombieFake().getX(),zombieFakeManager.getZombieFake().getY())){
                switch (i){
                    case 8:
                        for(int j = 0;j < 9;j++){
...
//            }
//        }
        if(projectileManager.getRealTimeCounter() == 120){
